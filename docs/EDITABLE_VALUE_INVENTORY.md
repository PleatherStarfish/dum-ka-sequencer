...
| `generator.dumka.subdivisionPalette` | Depth palette chips | order-insensitive unique subset of `2`, `3`, `5`, `7`; at most 2; refined working Subdivision ≤64 | `PatchGeneratorConfig.subdivisionPalette` | Persistence, required/apply structure, preview/playback working lattice | Structural/discrete |
...
| `generator.dumka.complexityFloor` | Mean-depth corridor floor | milli `0..100000`, ≤ ceiling (shown /1000) | `PatchGeneratorConfig.complexityFloor` | Persistence, preview/playback depth normalization and independent clamp trace | Current (`generator.dumka.complexityFloor`, cycleStart) |
| `generator.dumka.complexityCeiling` | Mean-depth corridor ceiling | milli `0..100000`, ≥ floor (shown /1000) | `PatchGeneratorConfig.complexityCeiling` | Persistence, preview/playback candidate admission and independent clamp trace | Current (`generator.dumka.complexityCeiling`, cycleStart) |
| `generator.dumka.placementBias` | Metric ↔ void placement slider | percent `0..100` | `PatchGeneratorConfig.placementBias` | Persistence, fixed-point Barlow/geometric candidate blend, deterministic replay | Current (`generator.dumka.placementBias`, cycleStart) |
...
| `generator.dumka.plan` | Evolve lane pins/ranges + inspector | ordered directive rows: family including directive-only Morph, cycles ≥1, intensity `0..100`, pacing, optional beat scope/options | `PatchGeneratorConfig.plan` | Persistence normalization/warnings, deterministic preview/playback fold, preview trace | Structural tuple; deliberately not an automation target |
...
| `generator.dumka.plan[*].options.complexityFloor/Ceiling` | Directive mean-depth override | paired milli values `0..100000`, floor ≤ ceiling, or both absent | `DirectiveOptions.complexityFloor` / `complexityCeiling` | Strict/tolerant persistence fences, deterministic depth normalization, clamp trace | Structural/discrete; inherits global rails when absent |
| `generator.dumka.plan[*].options.placementBias` | Directive placement override | nullable percent `0..100` | `DirectiveOptions.placementBias` | Deterministic per-row Barlow/geometric candidate blend | Structural/discrete; inherits global bias when absent |
| `generator.dumka.plan[*].options.subdivisionLevel` | Directive palette-level filter | nullable enabled palette prime `2`, `3`, `5`, or `7` | `DirectiveOptions.subdivisionLevel` | Candidate placements restricted to reduced denominators divisible by that prime | Structural/discrete; not automation |
| `generator.dumka.plan[*].options.morphTarget` | Morph target pattern | nonblank Dum-Ka notation; same beats as seed; required Subdivision divides working Subdivision; valid only for Morph | `DirectiveOptions.morphTarget` | Strict/tolerant validation, exact directed micro-step fold, endpoint preview/playback | Structural text; not automation |
