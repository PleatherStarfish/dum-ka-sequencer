...
# `+nightly` here too: a bare `cargo fuzz` resolves the repo toolchain pin
# (rust-toolchain.toml, stable) and in CI would auto-download a full stable
# toolchain just for this version check.
...
run_target() {
  ...
  echo "==> ${target} (${FUZZ_RUNS} runs, max_len=${max_len}, timeout=${FUZZ_TIMEOUT}s)"
